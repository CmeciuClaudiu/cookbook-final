...
export class RecipesService {
  ...
  getIngredients(): Observable<IngredientModel> {
    return this.http.get<IngredientModel>('http://claudiuc.ddns.net:5000/api/ingredients');
  }
  ...
  postRecipe(recipe: RecipeModel, username: string, password: string): Observable<string> {
    ...
    return this.http.post<string>('http://claudiuc.ddns.net:5000/api/post', recipe, { responseType: 'text' as 'json', params: params })
  }

  getDtoRecipes(dishType: number): Observable<DtoRecipeModel[]> {
    let params = new HttpParams().set("dishType", dishType);

    return this.http.get<DtoRecipeModel[]>('http://claudiuc.ddns.net:5000/api/dtorecipe', { params: params });
  }

  getRecipe(id: any): Observable<RecipeModel> {
    let params = new HttpParams().set("id", id);

    return this.http.get<RecipeModel>('http://claudiuc.ddns.net:5000/api/recipe', { params: params });
  }
}
